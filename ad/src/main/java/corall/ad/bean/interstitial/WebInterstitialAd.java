...
public class WebInterstitialAd extends RawInterstitialAd {
    private String id;
    private String url;
    ...
    public WebInterstitialAd(String id, String url) {
        this.id = id;
        this.url = url;

        URL_KEY = getString(R.string.web_in_web_url);
        AD_PLACE_ID = getString(R.string.web_in_place_id);
    }

    @Override
    public String getPlatform() {
        return AdsContants.AD_PLATFORM_WEB;
    }
    ...
    public void showByAct(Class clazz) {
        try {
            Intent intent = new Intent(CorApplication.getInstance(), clazz);
            intent.putExtra(URL_KEY, url);
            intent.putExtra(AD_PLACE_ID, getPlaceId());
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            CorApplication.getInstance().startActivity(intent);
            if (mListener != null) {
    ...
    private void initWebView(WebView wv) {
        wv.setWebViewClient(new AdWebViewClient(new IWebViewController() {
            ...
            public void onWebJump() {
                if (mListener != null) {
                    mListener.onClick(WebInterstitialAd.this);
                }
                close();
            }
        }));
